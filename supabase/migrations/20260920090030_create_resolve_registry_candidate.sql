-- Registry Sync P0, migration 4 of 4 — the atomic registry-write RPC, per
-- the approved final design (Section 3/4/9/10/14). The single, future
-- database entry point for manual/csv/apify alike — none of them write to
-- public.companies/public.company_sources/public.registry_sync_staging
-- directly, and this function never branches on p_discovery_source beyond
-- storing it (approved design Section 1).
--
-- One transactional RPC (Option B in the approved design's own A/B
-- comparison), not two separate company/source RPCs: company resolution
-- and source resolution happen inside ONE function body, called via ONE
-- PostgREST request, executing as ONE Postgres transaction — a source-side
-- failure automatically rolls back a just-created company. No
-- n8n-orchestrated multi-call sequence can ever observe a company with no
-- source.
--
-- Concurrency: no hard unique index is safe at the company-identity layer
-- (real name collisions exist and are legitimate — Accenture, Amazon), so
-- two concurrent callers resolving "the same" normalized company name are
-- serialized by pg_advisory_xact_lock(hashtext(...)) instead — the second
-- caller's own lookup, run after acquiring the lock, sees the first
-- caller's already-committed row. The source layer has a real, proven-safe
-- unique index (company_sources_identity_key, prior migration), so
-- INSERT ... ON CONFLICT DO NOTHING + re-select-on-race-loss (the exact
-- idiom already used by this schema's own create_analysis_task) is
-- sufficient there.
...
comment on function public.normalize_company_name(text) is
  'Lowercase, strip every non-alphanumeric character. Used only for company-identity matching inside resolve_registry_candidate() — deliberately NOT a generated column or a unique constraint (approved design Section 3/16): validated read-only against all 553 live companies, this signal alone produces exactly 2 collision groups (Accenture, Amazon) that are legitimately separate entities, so a hard uniqueness guarantee on it would be actively wrong.';
...
begin
  ...
  if p_discovery_source is null or p_discovery_source not in ('manual', 'csv', 'apify') then
    raise exception 'resolve_registry_candidate: p_discovery_source must be one of manual/csv/apify, got %', p_discovery_source
